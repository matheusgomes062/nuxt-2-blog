...
describe('ShareIcons.vue', () => {
  ...
  it('renders all social icons correctly', () => {
    const wrapper = mount(ShareIcons, { propsData: baseProps });
  ...
  it('generates the correct href for each social icon', () => {
    ...
    expect(links.at(0).attributes('href')).toBe(expectedLinks.Linkedin);
    expect(links.at(1).attributes('href')).toBe(expectedLinks.Twitter);
    expect(links.at(2).attributes('href')).toBe(expectedLinks.Facebook);
    expect(links.at(3).attributes('href')).toBe(expectedLinks.Gmail);
    expect(links.at(4).attributes('href')).toBe(expectedLinks.Pinterest);
  });
