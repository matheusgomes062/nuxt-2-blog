...
describe('Error.vue', () => {
  const createWrapper = () => {
    return mount(Error, {
      stubs: {
        NuxtLink: {
          props: ['to', 'target', 'aria-label'], // Match the props in the component
          template: `<a :href="to" :aria-label="ariaLabel"><slot /></a>`, // Use `ariaLabel` as it matches Vue's camelCase convention for props
        },
        Section: {
          props: ['id', 'fullHeight'],
          template: `<section :id="id" :class="{ 'h-full': fullHeight }"><slot /></section>`,
        },
      },
    });
  };
  ...
  it('renders the NuxtLink component with correct props', () => {
    ...
    expect(nuxtLink.attributes('aria-label')).toBe('Go back home.'); // Correctly bound attribute
  });
